- [ ] Implement Profile Sidebar
    <!-- github:start -->
    <details><summary>🔗 GitHub Integration</summary>
    
    - **Task ID:** 46
    - **State:** Open
    - **GitHub URL:** https://github.com/mallowigi/pkmnvuequiz/issues/46
    </details>
    <!-- github:end -->
- [ ] Missingno
    <!-- github:start -->
    <details><summary>🔗 GitHub Integration</summary>
    
    - **Task ID:** 52
    - **State:** Open
    - **GitHub URL:** https://github.com/mallowigi/pkmnvuequiz/issues/52
    - **Completed:** 1783661730165
    </details>
    <!-- github:end -->
- [ ] Prevent Leaderboards Spoofing ##leaderboards,firebase##
    <!-- github:start -->
    <details><summary>🔗 GitHub Integration</summary>
    
    - **Task ID:** 51
    - **State:** Open
    - **GitHub URL:** https://github.com/mallowigi/pkmnvuequiz/issues/51
    - **Completed:** 1783661731073
    </details>
    <!-- github:end -->
    <!-- ai_dialog:start -->
    <details><summary>🤖 AI Assistant Dialog</summary>
    
    I want to know how can I solve this issue: in the game we have the ability to load/save the current state. But we also have the feature of saving records to the leaderboards in firebase. This causes a problem: users could save state before finishing the game, finish the game again, and repeat, which would cause a lot of bad records in the database.
    
    Do you have a plan on how to tackle this issue?
    </details>
    <!-- ai_dialog:end -->
- [ ] Multiplayer ##next##
    <!-- github:start -->
    <details><summary>🔗 GitHub Integration</summary>
    
    - **Task ID:** 50
    - **State:** Open
    - **GitHub URL:** https://github.com/mallowigi/pkmnvuequiz/issues/50
    - **Completed:** 1783661731556
    </details>
    <!-- github:end -->
- [ ] PWA/Electron ##next##
    <!-- github:start -->
    <details><summary>🔗 GitHub Integration</summary>
    
    - **Task ID:** 49
    - **State:** Open
    - **GitHub URL:** https://github.com/mallowigi/pkmnvuequiz/issues/49
    </details>
    <!-- github:end -->
- [ ] AttackDex ##next##
    <!-- github:start -->
    <details><summary>🔗 GitHub Integration</summary>
    
    - **Task ID:** 48
    - **State:** Open
    - **GitHub URL:** https://github.com/mallowigi/pkmnvuequiz/issues/48
    </details>
    <!-- github:end -->
- [ ] Leaderboards Admin ##admin,leaderboards##
    <!-- comment:start -->
    <details><summary>Show Comment</summary>
    
    Need to be able to edit LeaderBoards on the fly
    </details>
    <!-- comment:end -->
    <!-- github:start -->
    <details><summary>🔗 GitHub Integration</summary>
    
    - **Task ID:** 47
    - **State:** Open
    - **GitHub URL:** https://github.com/mallowigi/pkmnvuequiz/issues/47
    - **Completed:** 1783661734344
    </details>
    <!-- github:end -->
- [ ] Leaderboard Stats ##next,leaderboards##
    <!-- github:start -->
    <details><summary>🔗 GitHub Integration</summary>
    
    - **Task ID:** 53
    - **State:** Open
    - **GitHub URL:** https://github.com/mallowigi/pkmnvuequiz/issues/53
    </details>
    <!-- github:end -->
